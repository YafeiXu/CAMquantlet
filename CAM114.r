##### CAM114-3 yafei.xu@hu-berlin.de
...
dateC = c("1986", "1987", "1988", "1989", "1990", "1991", "1992", "1993", "1994", 
...
plot(seq(1, length(DAXreturn), length.out = length(DAXreturn)), seq(min(DAXreturn), max(DAXreturn), length.out = length(DAXreturn)), 
     xlab = "", ylab = "",  main = expression("DAX return ("*r[t] == log(P[t] / P[t-1])*")"), col = "White", 
     axes = FALSE,  xaxt = "n",  yaxt = "n")
box() 
axis(1, at = At, labels = F)
axis(2, at = seq(min(DAXreturn), max(DAXreturn), length.out = 13)) 
mtext(expression(r[t]),  side = 2,  line = 2.7,  at = seq(min(DAXreturn), max(DAXreturn), length.out = 13)[7], font = 2, cex = 1)  
